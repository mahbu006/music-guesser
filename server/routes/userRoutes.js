...
module.exports = app => {
  ...
  app.get("/user/single/stats/genre/:genre", (req,res) => {
    try {
      const gamesPlayed = await SingleScore.find({ userId: req.user._id, genre: req.params.genre });
      //games played
      //highest score in each mode
    } catch (err) {
      res.status(400).send({ error: "User stats for this genre couldn't be found." });
    }
  })
};
